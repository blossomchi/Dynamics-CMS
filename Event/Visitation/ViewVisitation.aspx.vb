...
Partial Class ViewVisitation
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Response.Redirect("Visitation.aspx")
    End Sub
    Protected Sub GVAllAttendance_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GVAllAttendance.RowCommand
        If e.CommandName = "DeleteMinstersID" Then
            If e.CommandArgument <> Nothing And IsNumeric(e.CommandArgument) Then
                ...
                Dim Name = madd.VisitationTables.Where(Function(s) s.Sn = SNID).SingleOrDefault.Name

                Dim checkFunctional = (From s In madd.VisitationTables
                                       Where (s.Sn = SNID)
                                       Select s).Any

                If checkFunctional Then
                    Dim deleteInfo = (From s In madd.VisitationTables
                                      Where (s.Sn = SNID)
                                      Select s).SingleOrDefault

                    madd.VisitationTables.DeleteOnSubmit(deleteInfo)
                    madd.SubmitChanges()
                End If
