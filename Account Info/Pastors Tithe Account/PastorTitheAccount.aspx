<%@ Page Language="VB" AutoEventWireup="false" CodeFile="PastorTitheAccount.aspx.vb" Inherits="Tithe_Account" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
      <title>Pastors Tithe Account</title>
        ...
        <div>
            <asp:GridView ID="GridTithe" runat="server" Width="1150px" DataSourceID="SqlDataSource1" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" CellPadding="6" ForeColor="#333333" GridLines="None" PageSize="20" ShowFooter="True" style="font-size: small; font-weight: 700;" CellSpacing="2" BorderStyle="Solid">
                ...
                <Columns>
                    ...
                    <asp:BoundField DataField="Pastors Name" HeaderText="Pastors Name" SortExpression="Pastors Name" />
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ChurchdbConnectionString %>" SelectCommand="SELECT PastorsAccountTable.TranscationDate, MinisterTable.Name AS [Pastors Name], PastorsAccountTable.AccountName, PastorsAccountTable.Accountdetails, PastorsAccountTable.Transcationtype, PastorsAccountTable.Amount, PastorsAccountTable.Remark, PastorsAccountTable.Datecreated, MembershipTable.FirstName + ' ' + MembershipTable.LastName AS Enterby FROM MembershipTable INNER JOIN PastorsAccountTable ON MembershipTable.Memberid = PastorsAccountTable.StaffId INNER JOIN MinisterTable ON PastorsAccountTable.MinsterID = MinisterTable.Ministerid ORDER BY PastorsAccountTable.TranscationDate DESC">
            </asp:SqlDataSource>
            ...
            <asp:SqlDataSource ID="SqlTitle" runat="server" ConnectionString="<%$ ConnectionStrings:ChurchdbConnectionString %>" SelectCommand="SELECT PastorsAccountTable.TranscationDate, MinisterTable.Name AS [Pastors Name], PastorsAccountTable.AccountName, PastorsAccountTable.Accountdetails, PastorsAccountTable.Transcationtype, PastorsAccountTable.Amount, PastorsAccountTable.Remark, PastorsAccountTable.Datecreated, MembershipTable.FirstName + ' ' + MembershipTable.LastName AS Enterby FROM MembershipTable INNER JOIN PastorsAccountTable ON MembershipTable.Memberid = PastorsAccountTable.StaffId INNER JOIN MinisterTable ON PastorsAccountTable.MinsterID = MinisterTable.Ministerid WHERE (PastorsAccountTable.TranscationDate = @transdate) AND (MinisterTable.ChurchDetailID = @chu) ORDER BY PastorsAccountTable.TranscationDate DESC">
                <SelectParameters>
            ...
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ChurchdbConnectionString %>" SelectCommand="SELECT PastorsAccountTable.TranscationDate, MinisterTable.Name AS [Pastors Name], PastorsAccountTable.AccountName, PastorsAccountTable.Accountdetails, PastorsAccountTable.Transcationtype, PastorsAccountTable.Amount, PastorsAccountTable.Remark, PastorsAccountTable.Datecreated, MembershipTable.FirstName + ' ' + MembershipTable.LastName AS Enterby FROM MembershipTable INNER JOIN PastorsAccountTable ON MembershipTable.Memberid = PastorsAccountTable.StaffId INNER JOIN MinisterTable ON PastorsAccountTable.MinsterID = MinisterTable.Ministerid WHERE (PastorsAccountTable.Datecreated = @transdate) AND (MinisterTable.ChurchDetailID = @chu) ORDER BY PastorsAccountTable.TranscationDate DESC">
                <SelectParameters>
                    <asp:ControlParameter ControlID="txtSelectedDate" Name="transdate" PropertyName="Text" />
                    <asp:ControlParameter ControlID="txtchurchID" Name="chu" PropertyName="Text" />
                </SelectParameters>
